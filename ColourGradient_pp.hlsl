...
float4 main(PostProcessingInput input) : SV_Target
{
	...
	// Convert to HSL
	float3 blueHSL = RGBtoHSL(blue);
	float3 yellowHSL = RGBtoHSL(yellow);

	// Increase the hue component
	blueHSL.r += gColourTimer / 10;
	yellowHSL.r += gColourTimer / 10;
	...
	float3 colour = SceneTexture.Sample(PointSample, input.sceneUV).rgb * gradientColour;
